...
router.post('/register', (req, res) => {
  ...
  const existing = dbApi.get('SELECT id FROM users WHERE username = ?', [username]);
  if (existing) return res.status(409).json({ error: '用户名已存在' });

  const hashed = bcrypt.hashSync(password, 10);
  const result = dbApi.run('INSERT INTO users (username, password) VALUES (?, ?)', [username, hashed]);
...
router.post('/login', (req, res) => {
  ...
  const user = dbApi.get('SELECT * FROM users WHERE username = ?', [username]);
